...
Schema preserves original language, locale, source-country/region, multilingual entities, and UTC timestamps. The reader uses a validated `Accept-Language` preference for document language and server-rendered date/time formatting, then allows an explicit browser-localized English/Spanish UI preference stored in a secure, same-site cookie. Core reader chrome (home, offline state, followed brief/alerts, and story evidence headings) is translated; publisher headlines, quotations, evidence notes, and operator-entered content remain original-language records and are never machine-translated as though they were source material. Unsupported UI languages fall back to English. Political orientation remains contextual; “Left/Centre/Right” is a display projection only where meaningful.
