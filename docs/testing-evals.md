...
Published claims must have a linked story report and an attributed note. Timeline items currently represent publication dates only. Diversity cards disclose the count of sources with recorded ownership; they do not calculate independence or political balance.
...
`evals/blindspots.json` is a versioned regression set for the first geographic coverage-gap check. It verifies both a qualified observed gap and an adequate sample that includes the expected country. Unit cases also prove that undersized samples are withheld. The method requires an explicit reviewed target, at least five reports, three publications, two countries, 80% coverage review, and complete country/reporting-chain provenance; it does not infer motive, balance, or source quality.
