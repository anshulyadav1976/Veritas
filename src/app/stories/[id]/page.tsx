  ...
  return <main id="main-content" className="shell">
    ...
    <article className="story-detail">
      ...
      <section className="claims" aria-labelledby="claims-heading">
        ...
        {story.claims.length === 0 ? <p className="empty-copy">No reviewed claim records have been published. Reports above are the available evidence trail.</p> : <ol className="claim-list">{story.claims.map((claim) => <li key={claim.id}><p className="eyebrow">{statusLabel(claim.status)} · {statusLabel(claim.claimType)} · {claim.analysisVersion}</p><h3>{claim.text}</h3>{claim.evidence.map((evidence) => <div key={evidence.id}><p>Publisher report · {evidence.stance}: <a href={evidence.articleUrl} target="_blank" rel="noreferrer">{evidence.sourceName} — {evidence.articleTitle}</a></p>{evidence.evidenceSpan && <blockquote>{evidence.evidenceSpan}</blockquote>}<p className="metrics">Review note: {evidence.note}</p></div>)}{claim.primaryMaterials.map((material)=><p key={material.id}>Primary material · {material.stance}: <a href={material.url} target="_blank" rel="noreferrer">{material.title} ↗</a><br/><span className="metrics">{material.note}</span></p>)}</li>)}</ol>}
      </section>
